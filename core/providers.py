...
智谱使用独立的 CHATHELP_ZHIPU_API_KEY；旧判断/起草供应商保留 JEV_API_KEY/LLM_API_KEY。
"""
...
BIGMODEL_ENV = "CHATHELP_ZHIPU_API_KEY"
...
JEV_PROVIDERS = {
    "zhipu": _Jev("智谱免费", BIGMODEL_MODEL),
...
DRAFT_PROVIDERS = {  # 首次安装默认智谱，已有配置继续保留。
    "zhipu": _Draft("智谱免费", "openai", BIGMODEL_BASE, BIGMODEL_MODEL, _NONE),
...
ENV_VARS = sorted({BIGMODEL_ENV, JEV_ENV, LLM_ENV, *LEGACY.values()})
...
if __name__ == "__main__":
    ...
    assert next(iter(DRAFT_PROVIDERS)) == "zhipu"
    ...
    # 全程只有两把 key，脱敏还得管老名字
    assert BIGMODEL_ENV in ENV_VARS
    print("providers ok")
